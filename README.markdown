...
我们在setup中结构emit：`setup(props, { emit })`，因此在setupComponent中调用setup时要传入`emit`。
...
在provide函数中，拿到当前instance的provides和parent的provides，如果两者相同，说明在该组件中还没有provide新的值，初始化`currentInstance.provides`为空对象，并将它的原型对象指向`parent.provides`：利用`provides = currentInstance.provides = Object.create(parentProvides);`
