...
+ 文本（`"hi,"`）在ast节点中的content属性中保存，`getNode`函数用来将`content`解析出来并凭借在`code`后
+ 最后返回对象，具有code属性
...
`getNode`函数用来获取content内容，使用`switch`区分了不同类型节点的情况。

其中genInterpolation处理插值，还要继续调用getNode拿到最里面的content
